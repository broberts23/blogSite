...
The implication for automation is that you need two distinct execution targets: a domain controller for the AD provisioning steps (`New-ADServiceAccount`, `Set-ADServiceAccount`, `Start-ADServiceAccountMigration`), and a member server/s for the local service configuration steps (`Set-ItemProperty` on the registry, `Invoke-CimMethod` on the Win32\_Service class). The Migration Factory handles this by accepting both `domainController` and `targetHost` in the migration request, and spawning two separate WinRM runners:
...
*   [My Repo](https://github.com/broberts23/vsCode/tree/main/project-dmsa-migration-factory)
